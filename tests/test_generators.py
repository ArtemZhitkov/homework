...
def test_filter_by_currency_usd(data_transaction: List[dict]) -> None:
    assert next(filter_by_currency(data_transaction, "USD")) == {'id': 939719570, 'state': 'EXECUTED',
...
def test_filter_by_currency_rub(data_transaction: List[dict]) -> None:
    assert next(filter_by_currency(data_transaction, "RUB")) == {'id': 873106923, 'state': 'EXECUTED',
